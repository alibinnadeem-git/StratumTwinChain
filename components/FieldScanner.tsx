...
export default function FieldScanner(){
  ...
  async function start(){
    ...
    try{
      ...
      setStarting(false);setScanning(true);setMessage('Camera active. Point it at a QR or equipment barcode.');
  ...
  return <div className="phone">
    ...
    <div className="scan-box" style={{position:'relative',overflow:'hidden'}}>
      <video ref={videoRef} playsInline muted autoPlay style={{width:'100%',height:'100%',objectFit:'cover',position:'absolute',inset:0,display:scanning?'block':'none'}}/>{!scanning&&<><div className="scan-line"/><b>{detected?'Equipment detected':'Scan equipment'}</b><span>{detected||'QR · Barcode · Serial'}</span></>}
    </div>
    ...
    <button type="button" disabled={starting} onClick={scanning?stop:start}>{scanning?'Stop camera':'Open scanner'}</button>
