...
export default function FieldScanner(){
  ...
  return <div className="phone">
    ...
    <div style={{display:'grid',gap:8,marginTop:10}}>
      ...
      <button type="button" onClick={()=>identify()} disabled={!value.trim()||resolving} style={{opacity:(!value.trim()||resolving)?.55:1}}>{resolving?'Resolving…':'Identify asset'}</button>
    </div>
    {asset&&<div className="phone-card" style={{marginTop:10}}><small>{asset.administratively_archived?'ADMINISTRATIVELY ARCHIVED':'LIVE TENANT ASSET'}</small><strong>{asset.asset_code} · {asset.name}</strong><span>{asset.site_name} · {asset.location_label||'Location pending'}</span>{asset.administratively_archived&&<span>{asset.archive_reason||'Archived by an authorized administrator.'}</span>}<div className="button-row" style={{marginTop:10}}><Link className="action" href={`/assets/${encodeURIComponent(asset.id)}`}>Open Passport</Link><Link className="action" href={`/verify?q=${encodeURIComponent(asset.asset_code)}`}>Public verification</Link></div><button type="button" onClick={continueInspection} disabled={Boolean(asset.administratively_archived)} style={{width:'100%',marginTop:8,opacity:asset.administratively_archived?.55:1}}>Continue inspection</button></div>}
