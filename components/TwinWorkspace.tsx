...
const LAYERS:{id:LayerId;name:string;short:string}[]=[
 ...
 {id:'L8',name:'Trust / DIRS',short:'Immutable record proof'}
];
...
export default function TwinWorkspace({assets,referenceModelUrl}:{assets:TwinAsset[];referenceModelUrl?:string}){
 ...
 async function onFile(e:ChangeEvent<HTMLInputElement>){
  ...
  if(['glb','gltf'].includes(ext)){
   ...
   setMessage('3D model loaded. Geometry is being classified into architectural, physical and logical layers; matched equipment is bound to STRATUM assets and DIRS proof state.');
  }else{
   setModelUrl('');
 ...
 async function ingest(){
  ...
  setBusy(true);setMessage('Registering candidate assets and creating immutable DIRS lifecycle records…');
  try{
   ...
   const out=j as IngestResult;setMessage(`${out.assets.length} durable asset identities created under ${out.twin.assetCode}. Their immutable DIRS records are hashed and awaiting approval.`);setTimeout(()=>location.reload(),1100);
 ...
 return <div className={styles.workspace}>
  ...
  <div className={styles.stageWrap}><div className={styles.canvas} ref={mount}/><div className={styles.legend}><span><i className={styles.green}/>DIRS verified</span><span><i className={styles.blue}/>Registered / pending</span><span><i className={styles.orange}/>Design object</span><span><i className={styles.purple}/>Immutable record marker</span></div><div className={styles.orbit}>Drag to orbit · scroll to zoom · click equipment to inspect</div></div>
  <aside className={styles.panel}><div className={styles.metrics}><div><b>{assets.length}</b><span>Asset identities</span></div><div><b>{verifiedCount}</b><span>DIRS verified</span></div><div><b>{objects.length}</b><span>Twin objects</span></div><div><b>{boundCount}</b><span>Bound objects</span></div></div>
   ...
   {selected?<><div className={styles.passportHead}><span>VERIFICATION PASSPORT</span><em className={selected.ledger_block_height?styles.seal:styles.pending}>{selected.ledger_block_height?'DIRS VERIFIED':'PENDING'}</em></div><h2>{selected.name}</h2><p>{selected.asset_code} · {selected.asset_type}</p><div className={styles.facts}><div><span>Site</span><b>{selected.site_name}</b></div><div><span>System</span><b>{selected.system_name||'Unassigned'}</b></div><div><span>Manufacturer</span><b>{selected.manufacturer_name||'Not recorded'}</b></div><div><span>Serial</span><b>{selected.serial_number||'Pending'}</b></div><div><span>Lifecycle</span><b>{selected.status}</b></div><div><span>Latest event</span><b>{selected.latest_event_type||'REGISTER_ASSET'}</b></div></div><div className={styles.chainBox}><span>DIGITAL IMMUTABLE RECORDS SYSTEM (DIRS)</span><strong>{selected.ledger_network||'stratum-devnet-1'}</strong><small>{selected.ledger_block_height?`Immutable record #${selected.ledger_block_height}`:'Record awaiting approval / finalization'}</small><small>{selected.ledger_tx_hash||'Hashes only — private model and evidence remain private'}</small></div><div className={styles.actions}><Link href={`/assets/${selected.id}`}>Open passport</Link><Link href="/dirs">Verify in DIRS</Link></div></>:<div className={styles.empty}>Select an asset to inspect its passport.</div>}
