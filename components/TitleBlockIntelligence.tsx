...
export default function TitleBlockIntelligence(){
    ...
    try{
      ...
      for(const file of pdfs){
        ...
        for(const candidate of candidates){
          ...
          existing.set(keyOf(candidate),prior?.reviewState==='CONFIRMED'?{...candidate,reviewState:'CONFIRMED',confirmedAt:prior.confirmedAt,alignmentEligible:false}:candidate);
        }
        next=[...existing.values()].sort((a,b)=>a.sourceName.localeCompare(b.sourceName)||a.page-b.page);
        writeStored(next);setItems(next);
      }
      setMessage(`Title-block scan complete: ${next.length} sheet identity candidate${next.length===1?'':'s'} · ${next.filter(item=>item.reviewState==='CONFIRMED').length} human-confirmed. Confirmation does not establish alignment, Verified state or PoVI finality.`);
  ...
  function updateReview(target:StoredSheetIdentity,confirmedState:boolean){
    const next=items.map(item=>keyOf(item)===keyOf(target)?{...item,reviewState:confirmedState?'CONFIRMED':'CANDIDATE',confirmedAt:confirmedState?new Date().toISOString():undefined,alignmentEligible:false as const}:item) as StoredSheetIdentity[];
    setItems(next);writeStored(next);
    setMessage(confirmedState?'Sheet identity confirmed for review. Automatic alignment is still disabled until a separate alignment validation step.':'Sheet identity returned to candidate review state.');
  }
  ...
  return <section className="card" style={{marginTop:16}}>
    <div className="section-head"><div><div className="eyebrow">Title-block intelligence · Human reviewed</div><h2>Resolve sheet identity before multi-sheet alignment</h2><p className="muted">PDF text and position are used to propose sheet number, title, revision, issue date and discipline. Every result remains a candidate until explicitly confirmed. Confirmation never establishes geometry alignment, Verified state, DIR finality or physical truth.</p></div><span className="pending">{confirmed}/{items.length} CONFIRMED</span></div>
    ...
    {items.length>0&&<div style={{display:'grid',gap:10,marginTop:12}}>{items.map(item=><article className="card" key={keyOf(item)} style={{padding:14}}>
      ...
      <div className="grid two" style={{marginTop:8}}><div><div className="label">Sheet number</div><b>{item.sheetNumber.value||'Unresolved'}</b><small style={{display:'block'}}>confidence {Math.round(item.sheetNumber.confidence*100)}% · {item.sheetNumber.method}</small></div><div><div className="label">Sheet title</div><b>{item.sheetTitle.value||'Unresolved'}</b><small style={{display:'block'}}>confidence {Math.round(item.sheetTitle.confidence*100)}% · {item.sheetTitle.method}</small></div><div><div className="label">Discipline</div><b>{item.discipline.value||'Unresolved'}</b></div><div><div className="label">Revision / issue date</div><b>{item.revision.value||'—'} · {item.issueDate.value||'—'}</b></div></div>
      <p className="muted" style={{marginBottom:8}}>Evidence: {[...new Set([...item.sheetNumber.evidence,...item.sheetTitle.evidence,...item.revision.evidence,...item.issueDate.evidence])].filter(Boolean).slice(0,8).join(' · ')||'No strong labeled evidence; review required.'}</p>
