...
export default function CompiledGraphViewer({registeredAssets=[]}:{registeredAssets?:RegisteredSpatialAsset[]}){
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      const hitEntity=(ev:PointerEvent)=>{const rect=renderer.domElement.getBoundingClientRect();if(!rect.width||!rect.height)return null;pointer.x=((ev.clientX-rect.left)/rect.width)*2-1;pointer.y=-((ev.clientY-rect.top)/rect.height)*2+1;ray.setFromCamera(pointer,camera);const hit=ray.intersectObjects(clickable,true)[0];let node:any=hit?.object;while(node&&!node.userData?.entity)node=node.parent;return node?.userData?.entity as Entity|undefined};
