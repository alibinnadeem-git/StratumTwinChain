...
export default function CompilerWorkspace(){
 const [files,setFiles]=useState<SourceFile[]>([]);const [entities,setEntities]=useState<GraphEntity[]>([]);const [busy,setBusy]=useState(false);const [message,setMessage]=useState('');
 const totals=useMemo(()=>({files:files.length,bytes:files.reduce((n,f)=>n+f.size,0),parsed:files.filter(f=>f.state==='parsed').length,entities:entities.length,assets:entities.filter(e=>e.layer==='L2'&&e.kind!=='line').length}),[files,entities]);
 ...
 function saveGraph(nextFiles:SourceFile[],nextEntities:GraphEntity[]){const graph:CompiledGraph={version:'0.5',createdAt:new Date().toISOString(),sources:nextFiles.map(f=>({name:f.name,ext:f.ext,sha256:f.sha256,discipline:f.discipline})),entities:nextEntities,stats:{L0:nextFiles.length,L1:nextEntities.filter(e=>e.layer==='L1').length,L2:nextEntities.filter(e=>e.layer==='L2').length,L3:nextEntities.filter(e=>e.layer==='L3').length,L4:nextEntities.filter(e=>e.layer==='L4').length}};try{localStorage.setItem('stratum_compiled_graph',JSON.stringify(graph))}catch{} }
 async function addFiles(e:ChangeEvent<HTMLInputElement>){const incoming=Array.from(e.target.files||[]);if(!incoming.length)return;setBusy(true);let newFiles=[...files],newEntities=[...entities];
  for(const file of incoming){const ext=(file.name.split('.').pop()||'').toLowerCase(),buf=await file.arrayBuffer(),digest=await sha(buf),discipline=classify(file.name);let sf:SourceFile={name:file.name,size:file.size,ext,discipline,sha256:digest,state:'review',summary:'Format requires review',entities:0};
   try{if(ext==='pdf'){const parsed=await parsePdf(file);newEntities=[...newEntities,...parsed.entities];sf={...sf,state:'parsed',summary:parsed.summary,entities:parsed.entities.length,pages:parsed.pages,vectors:parsed.vectors,textItems:parsed.textItems}}
    else if(ext==='dxf'){const parsed=parseDxf(file,await file.text());newEntities=[...newEntities,...parsed.entities];sf={...sf,state:'parsed',summary:parsed.summary,entities:parsed.entities.length}}
  ...
  setBusy(false);setMessage(`Compilation updated: ${newFiles.filter(f=>f.state==='parsed').length}/${newFiles.length} sources parsed, ${newEntities.length} structured graph entities. The graph is now available to STRATUM Twin in this browser.`);e.target.value='';
 }
 return <div style={{marginTop:18}}>
  <div className="grid two"><div className="card"><div className="eyebrow">Engineering Source Ingestion</div><h2>Drop the project set</h2><label style={{display:'grid',placeItems:'center',minHeight:220,border:'1px dashed #397391',borderRadius:16,background:'#07131f',cursor:'pointer',textAlign:'center',padding:24}}><div><div style={{fontSize:34}}>⌁</div><b>{busy?'Parsing engineering sources…':'Drag / choose CAD, BIM, PDF or imagery'}</b><p className="subtitle" style={{margin:'8px auto 0'}}>PDF + DXF parse now · DWG/IFC/RVT preserved for native adapters · GLB/GLTF direct to Twin</p></div><input hidden multiple type="file" accept=".pdf,.dwg,.dxf,.ifc,.rvt,.glb,.gltf,.png,.jpg,.jpeg" onChange={addFiles}/></label>{message&&<div className="notice"><strong>COMPILED</strong><span>{message}</span></div>}<div style={{marginTop:15}}>{files.map((f,i)=><div className="file-row" key={`${f.name}-${i}`}><div className="file-icon">{f.ext.toUpperCase()}</div><div><strong>{f.name}</strong><small>{f.discipline} · {(f.size/1024/1024).toFixed(2)} MB · SHA-256 {f.sha256.slice(0,12)}…</small><small>{f.summary}</small></div><span className={f.state==='parsed'?'proof':'pending'}>{f.state.toUpperCase()}</span></div>)}</div></div>
