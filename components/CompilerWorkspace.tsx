...
import {protectSpatialGraph,readCurrentSpatialGraph,replaceCurrentSpatialGraph} from '../lib/spatial-browser-recovery';
...
export default function CompilerWorkspace(){
 ...
 useEffect(()=>{const restore=()=>{try{const raw=localStorage.getItem('stratum_compiled_graph');if(!raw)return;const graph=JSON.parse(raw);if(!Array.isArray(graph.sources)||!Array.isArray(graph.entities))throw new Error();setEntities(graph.entities);const restored=graph.sources.map((source:CompiledGraph['sources'][number]&Partial<SourceFile>)=>({...source,size:source.size||0,state:(source.state||'review') as ParseState,summary:source.summary||'Restored saved source; extraction and alignment remain subject to review.',entities:Number.isFinite(Number(source.entities))?Number(source.entities):graph.entities.filter((e:GraphEntity)=>e.meta?.sourceSha256===source.sha256||e.source===source.name).length,floor:source.floor||'UNRESOLVED',elevation:source.elevation||0}));setFiles(current=>{const existing=new Map(current.map(item=>[item.sha256,item]));return restored.map((item:SourceFile)=>{const prior=existing.get(item.sha256);return prior?{...item,...prior}:item})});}catch{setMessage('Saved graph could not be read. Existing data has been preserved.');}};restore();window.addEventListener('stratum:graph-updated',restore);return()=>window.removeEventListener('stratum:graph-updated',restore);},[]);
 ...
 function saveGraph(nextFiles:SourceFile[],rawEntities:GraphEntity[]){
  const saved=JSON.parse(localStorage.getItem('stratum_compiled_graph')||'{}');
  if(saved.entities&&!Array.isArray(saved.entities))throw new Error('Saved graph is invalid; import was not written.');
  const current:GraphEntity[]=saved.entities||[];
