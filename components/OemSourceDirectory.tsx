...
export default function OemSourceDirectory(){
 ...
 const models=new Map(DEFAULT_ELECTRICAL_MODEL_REGISTRY.map(item=>[item.componentKey,item]));
 const visible=useMemo(()=>OEM_SOURCES.filter(source=>(evidence==='ALL'||source.evidence===evidence)&&`${source.manufacturer} ${source.families.join(' ')} ${source.fields.join(' ')} ${source.formats.join(' ')}`.toLowerCase().includes(search.trim().toLowerCase())),[search,evidence]);
 ...
 return <section className={styles.directory} aria-label="OEM source directory">
  ...
  <div className={styles.grid}>{visible.map(source=><article className={styles.card} key={source.id}>
   ...
   {source.componentKeys.length>0&&<div className={styles.detail}><b>Library classes</b><span>{source.componentKeys.map((key,i)=><span key={key}>{i>0?' · ':''}<a href={`#${key}`}>{components.get(key)||key}</a>{models.get(key)?.modelUrl?` (${models.get(key)?.geometryStatus==='LICENSED_COMMUNITY'?'community model':'STRATUM visualization'})`:''}</span>)}</span></div>}
