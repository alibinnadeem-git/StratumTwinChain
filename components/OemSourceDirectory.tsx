...
export default function OemSourceDirectory(){
 ...
 return <section className={styles.directory} aria-label="OEM source directory">
  ...
  <div className={styles.grid}>{visible.map(source=>{const binding=oemModelBinding(source.id,registry);return <article className={styles.card} key={source.id}>
   ...
   {OEM_CAD_CANDIDATES.filter(item=>item.sourceId===source.id).map(item=><div className={styles.detail} key={item.id}><b>Exact product · {item.sku}</b><span><a href={item.productUrl} target="_blank" rel="noopener noreferrer">{item.product} ↗</a> · {item.cadFormat} · {item.status==='GLB_APPROVED'?'Manufacturer CAD converted; exact SKU model active':item.status==='CAD_DOWNLOAD_IDENTIFIED'?'CAD download identified; file not imported':'Manufacturer product identified; CAD file not imported'}{item.dimensionsMeters?` · W × H × D: ${item.dimensionsMeters.map(n=>`${n*1000} mm`).join(' × ')}`:''}{item.cadUrl&&<> · <a href={item.cadUrl} target="_blank" rel="noopener noreferrer">Original CAD ↗</a></>}</span></div>)}
