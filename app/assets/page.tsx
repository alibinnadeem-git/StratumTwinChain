...
export default async function Assets(){
 ...
 if(!backendOnline)assets=demoAssets.map(a=>({id:a.id,name:a.name,asset_code:a.id,asset_type:a.type,manufacturer_name:a.manufacturer,serial_number:a.serial,system_name:a.system,site_name:a.site,location_label:a.location,status:a.status,project_code:a.project,latest_event_type:a.stages.slice().reverse().find(s=>s.status==='verified')?.stage||'REGISTER_ASSET',anchored_at:a.commissionedAt||a.installedAt||null,ledger_block_height:a.block||null}));
 return <>
 <div className="page-head"><div><div className="eyebrow">Asset Registry</div><h1 className="title">Asset passports</h1><p className="subtitle">Durable equipment identities with lifecycle evidence and Digital Immutable Records (DIR) verification.</p></div><div className="button-row"><Link className="ghost" href="/assets/archived">Archived assets</Link><Link className="action" href="/workflows">+ Register / verify</Link></div></div>
 {!backendOnline&&<div className="card" style={{marginBottom:16,borderColor:'#75592e'}}><div className="eyebrow">Registry available · live backend reconnecting</div><p className="subtitle" style={{marginTop:6}}>Reference passports remain available while the production database reconnects. Reference data is never presented as a live tenant registry.</p></div>}
