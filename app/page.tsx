...
export default function Home(){
 ...
 return <>
  ...
  <section className="grid two"><div><div className="section-head"><div><div className="eyebrow">Universal Asset View</div><h2>Infrastructure provenance at a glance</h2></div></div><AssetCard asset={assets[0]}/></div><div className="card"><div className="section-head"><div><div className="eyebrow">Activity</div><h3>Evidence and trust stream</h3></div></div><div className="timeline">{events.map(e=><div className="event" key={e.meta}><i className={`event-icon ${e.kind}`}>✓</i><div><strong>{e.title}</strong><small>{e.meta}</small></div></div>)}</div></div></section>
