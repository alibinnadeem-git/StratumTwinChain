...
export type LiveAssetRow={
  id:string;project_id:string;asset_code:string;asset_type:string;name:string;model:string|null;serial_number:string|null;location_label:string|null;status:string;qr_token:string;specifications:Record<string,unknown>;installed_at:Date|null;commissioned_at:Date|null;warranty_expires_at:Date|null;project_code:string;project_name:string;site_name:string;system_name:string|null;manufacturer_name:string|null;latest_event_id:string|null;latest_event_type:string|null;latest_event_status:string|null;ledger_network:string|null;ledger_tx_hash:string|null;ledger_block_height:string|null;anchored_at:Date|null;};

export async function liveAssets(){
  const session=await requireSession();
  const r=await query<LiveAssetRow>(`SELECT a.id,a.project_id,a.asset_code,a.asset_type,a.name,a.model,a.serial_number,a.location_label,a.status,a.qr_token::text,a.specifications,a.installed_at,a.commissioned_at,a.warranty_expires_at,p.project_code,p.name project_name,si.name site_name,sy.name system_name,m.name manufacturer_name,le.id::text latest_event_id,le.event_type::text latest_event_type,le.status::text latest_event_status,le.ledger_network,le.ledger_tx_hash,le.ledger_block_height::text,le.anchored_at FROM assets a JOIN projects p ON p.id=a.project_id JOIN sites si ON si.id=a.site_id LEFT JOIN systems sy ON sy.id=a.system_id LEFT JOIN manufacturers m ON m.id=a.manufacturer_id LEFT JOIN LATERAL (SELECT x.* FROM lifecycle_events x WHERE x.asset_id=a.id AND x.status='VERIFIED' ORDER BY x.anchored_at DESC NULLS LAST,x.occurred_at DESC LIMIT 1) le ON true WHERE a.organization_id=$1 ORDER BY COALESCE(le.anchored_at,a.created_at) DESC LIMIT 500`,[session.organizationId]);
  return r.rows;
}

export async function liveAsset(identifier:string){
  const session=await requireSession();
  const r=await query<LiveAssetRow>(`SELECT a.id,a.project_id,a.asset_code,a.asset_type,a.name,a.model,a.serial_number,a.location_label,a.status,a.qr_token::text,a.specifications,a.installed_at,a.commissioned_at,a.warranty_expires_at,p.project_code,p.name project_name,si.name site_name,sy.name system_name,m.name manufacturer_name,le.id::text latest_event_id,le.event_type::text latest_event_type,le.status::text latest_event_status,le.ledger_network,le.ledger_tx_hash,le.ledger_block_height::text,le.anchored_at FROM assets a JOIN projects p ON p.id=a.project_id JOIN sites si ON si.id=a.site_id LEFT JOIN systems sy ON sy.id=a.system_id LEFT JOIN manufacturers m ON m.id=a.manufacturer_id LEFT JOIN LATERAL (SELECT x.* FROM lifecycle_events x WHERE x.asset_id=a.id AND x.status='VERIFIED' ORDER BY x.anchored_at DESC NULLS LAST,x.occurred_at DESC LIMIT 1) le ON true WHERE (a.id::text=$1 OR a.asset_code=$1) AND a.organization_id=$2 LIMIT 1`,[identifier,session.organizationId]);
