...
// This module is a deterministic REFERENCE fixture until tenant-backed observation
// persistence is implemented. It must never be treated as live physical truth.
export const REALITY_VALIDATION_AUTHORITY='REFERENCE_ONLY' as const;
export const REALITY_TRUTH_BOUNDARY='OBSERVED_NEVER_OVERWRITES_VERIFIED' as const;
...
export function normalizeOEM(value:string){
 ...
 return {input:value,canonical:oemAliases[key]||value.trim(),matched:Boolean(oemAliases[key])};
}

// Reference baseline states only. In live mode these must come from governed,
// tenant-scoped records; Observed data may propose discrepancies but cannot mutate Verified.
...
export const observations:Observation[]=[
 {id:'OBS-2026-0913-001',assetId:'STR-AST-0009281',source:'PHONE',capturedAt:'2026-09-13T17:10:00.000Z',device:'Reference field phone · nameplate capture',modelVersion:'stratum-reality-reference-0.1',confidence:.98,evidenceHash:'sha256:9d0d3c1f8f88a21d',state:{manufacturer:'Square D',model:'MasterPact MTZ2',serial:'SE-MTZ2-928193',location:'Electrical Room 2A',attributes:{Voltage:'480 V',Current:'3000 A',Frequency:'60 Hz','Interrupt Rating':'100 kAIC'}}},
 {id:'OBS-2026-0913-002',assetId:'STR-AST-0009282',source:'360_CAMERA',capturedAt:'2026-09-13T17:22:00.000Z',device:'Reference 360 walkthrough',modelVersion:'stratum-reality-reference-0.1',confidence:.94,evidenceHash:'sha256:524d4412e1371880',state:{manufacturer:'Siemens',model:'Dry-Type 2500kVA',serial:'SIE-TX4-118203',location:'Utility Yard B',attributes:{Capacity:'2500 kVA',Primary:'12.47 kV',Secondary:'480 V',Cooling:'AN'}}},
 {id:'OBS-2026-0913-003',assetId:'STR-AST-0009283',source:'DRONE',capturedAt:'2026-09-13T17:38:00.000Z',device:'Reference RTK exterior capture',modelVersion:'stratum-reality-reference-0.1',confidence:.89,evidenceHash:'sha256:73ef0e91a21b58d9',state:{manufacturer:'ABB',model:'Terra 360',serial:'ABB-T360-551908',location:'Charging Yard C · 2.4 m east',attributes:{Power:'360 kW',Connectors:'CCS',Input:'480 V 3Φ',Network:'Ethernet / LTE'}}}
];

const severityFor=(field:string):RealityDiscrepancy['severity']=>['Current','Voltage','Primary','Secondary','Power','manufacturer','model'].includes(field)?'CRITICAL':field==='location'?'WARNING':'INFO';

export function compareObservedToDesigned(assetId:string):RealityDiscrepancy[]{
 const designed=designedStates[assetId],verified=verifiedStates[assetId],observation=observations.find(item=>item.assetId===assetId);
 if(!designed||!verified||!observation)return [];
 const identityRows:[string,AttributeValue,AttributeValue,AttributeValue][]=[
  ['manufacturer',designed.manufacturer,normalizeOEM(observation.state.manufacturer).canonical,verified.manufacturer],
  ['model',designed.model,observation.state.model,verified.model],
  ['serial',designed.serial,observation.state.serial,verified.serial],
  ['location',designed.location,observation.state.location,verified.location]
 ];
 ...
 return [...identityRows,...attributeRows]
  .filter(([,designedValue,observedValue])=>designedValue!==observedValue)
  .map(([field,designedValue,observedValue,verifiedValue],index)=>({
   id:`DISC-${observation.id}-${index+1}`,
   assetId,
   field,
   designed:designedValue,
   observed:observedValue,
   verified:verifiedValue,
   confidence:observation.confidence,
   severity:severityFor(field),
   reviewState:'REVIEW_REQUIRED',
   reason:`Observed ${field} differs from the reference approved design; Verified state remains unchanged pending authorized review.`,
...
export function realitySummary(){
 const compared=observations.length;
 const matched=compared-new Set(realityDiscrepancies.map(discrepancy=>discrepancy.assetId)).size;
 return {capturedAssets:observations.length,comparedAssets:compared,matchedAssets:matched,discrepancies:realityDiscrepancies.length,critical:realityDiscrepancies.filter(discrepancy=>discrepancy.severity==='CRITICAL').length,reconciliation:Math.round(matched/Math.max(compared,1)*100)};
}
