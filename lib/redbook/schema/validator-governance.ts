...
export const VALIDATOR_SET_STATE_PROFILE='STRATUM-VALIDATOR-SET-STATE/1' as const;
export const VALIDATOR_SET_STATE_DOMAIN='STRATUM/VALIDATOR_SET_STATE/1' as const;
export const VALIDATOR_SUBJECT_DOMAIN='STRATUM/VALIDATOR/SUBJECT/1' as const;
...
export const validatorSetChangeActionSchema=z.object({
 ...
 if(value.effectiveHeight<=value.approvedAtHeight){
  ctx.addIssue({code:z.ZodIssueCode.custom,path:['effectiveHeight'],message:'Validator governance actions must activate at a future height'});
 }
});
