...
export function verifyVoteMessageHash(vote:VerifyVoteProof){
 return canonicalHash({domain:POVI_VERIFY_DOMAIN,chainId:vote.chainId,height:vote.height,round:vote.round,step:'VERIFY',proposalHash:vote.proposalHash,validatorId:vote.validatorId,validatorSetRoot:vote.validatorSetRoot,protocolVersion:vote.protocolVersion});
}

export function roundChangeMessageHash(vote:RoundChangeVoteProof){
 return canonicalHash({domain:POVI_ROUND_CHANGE_DOMAIN,chainId:vote.chainId,height:vote.height,newRound:vote.newRound,validatorId:vote.validatorId,validatorSetRoot:vote.validatorSetRoot,protocolVersion:vote.protocolVersion,lockedDIR:vote.lockedDIR,lockedRound:vote.lockedRound,validDIR:vote.validDIR,validRound:vote.validRound,evidenceRefs:vote.evidenceRefs});
}
...
function verifyHashSignature(args:{validatorSet:SnapshotValidatorSet;height:number;validatorId:string;keyId:string;messageHash:string;signatureB64:string;}){
 ...
 const member=byId.get(args.validatorId); if(!member)return false;
 const key=activeConsensusKeyAtHeight(member,args.height); if(key.keyId!==args.keyId)return false;
 const publicKey=createPublicKey({key:Buffer.from(key.publicKeyDerB64,'base64'),format:'der',type:'spki'}); if(publicKey.asymmetricKeyType!=='ed25519')return false;
 return verifySignature(null,Buffer.from(args.messageHash,'hex'),publicKey,Buffer.from(args.signatureB64,'base64'));
}

export function verifyNilVerifyQuorum(args:{validatorSet:unknown;votes:unknown[];expectedChainId:string;height:number;round:number;expectedValidatorSetRoot:string;expectedProtocolVersion?:string;}){
 ...
 const seen=new Set<string>(); const validSigners:string[]=[];
 for(const vote of parsed){
  ...
  if(seen.has(vote.validatorId))throw new Error(`Duplicate NIL VERIFY signer ${vote.validatorId}`); seen.add(vote.validatorId);
 ...
 const {active}=activeMemberMap(validatorSet,args.height); const required=requiredPoviQuorum(active.length); const unique=[...new Set(validSigners)].sort();
...
export function verifyRoundChangeQuorumEvidence(args:{validatorSet:unknown;evidence:unknown;expectedChainId:string;expectedValidatorSetRoot:string;expectedProtocolVersion?:string;expectedCurrentRound?:number;}):VerifiedRoundChangeQuorumEvidence{
 const validatorSet=snapshotValidatorSetSchema.parse(args.validatorSet); const evidence=roundChangeQuorumEvidenceSchema.parse(args.evidence);
 ...
 const {active}=activeMemberMap(validatorSet,evidence.height); const required=requiredPoviQuorum(active.length);
 const seen=new Set<string>(); const validSigners:string[]=[];
 const lockClaims:VerifiedRoundChangeQuorumEvidence['lockClaims'][number][]=[]; const validValueClaims:VerifiedRoundChangeQuorumEvidence['validValueClaims'][number][]=[];
 for(const vote of evidence.roundChangeVotes){
  if(seen.has(vote.validatorId))throw new Error(`Duplicate ROUND_CHANGE signer ${vote.validatorId}`); seen.add(vote.validatorId);
  const expectedHash=roundChangeMessageHash(vote); if(vote.messageHash!==expectedHash)continue;
 ...
 const unique=[...new Set(validSigners)].sort(); if(unique.length<required)throw new Error(`ROUND_CHANGE quorum not met: ${unique.length}/${active.length}; ${required} required`);
 let nilQuorumVerified=false;
 if(evidence.priorRoundNILVotes.length>0){verifyNilVerifyQuorum({validatorSet,votes:evidence.priorRoundNILVotes,expectedChainId:args.expectedChainId,height:evidence.height,round:evidence.triggerRound,expectedValidatorSetRoot:computedRoot,expectedProtocolVersion:evidence.protocolVersion});nilQuorumVerified=true;}
 return{verified:true,evidence,activeValidatorCount:active.length,requiredQuorum:required,validSignerIds:Object.freeze(unique),nilQuorumVerified,lockClaims:Object.freeze(lockClaims),validValueClaims:Object.freeze(validValueClaims),safeUnlockAuthorized:false};
}
