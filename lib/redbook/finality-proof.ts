...
export function commitMessageHash(args:{
 chainId:string;height:number;round:number;proposalHash:string;stateRoot:string;validatorSetRoot:string;protocolVersion:string;
}){
...
export function finalizedDIRHash(header:DIRFinalityProof['header'],args:{proposalHash:string;signerIds:string[]}){
 return canonicalHash({
  domain:DIR_HASH_DOMAIN,
  header,
  finality:{
   proposalHash:args.proposalHash,
   stateRoot:header.stateRoot,
   validatorSetRoot:header.validatorSetRoot,
...
export function verifyDIRFinalityProof(args:{
