...
func TestActivationReadinessPreservesCandidateAndReportsRemainingBlockers(t *testing.T) {
	...
	if !got.GovernanceProofValid || !got.LocalIdentityMatch || !got.LocalConsensusKeyMatch { t.Fatal("expected governed identity/key match") }
	if got.GovernanceActivationDue { t.Fatal("activation must not be due before effective height") }
	if got.VoteAuthority || got.ReadyForVoteAuthority { t.Fatal("readiness verification must not grant vote authority") }
	if len(got.RemainingActivationBlockers) != 2 { t.Fatalf("expected two non-governance blockers, got %v", got.RemainingActivationBlockers) }
	if cfg.VoteAuthority || cfg.State != candidateState { t.Fatal("input candidate must remain unchanged") }
}
...
func TestActivationReadinessCanBecomeReadyOnlyWhenAllOtherBlockersClear(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if !got.GovernanceActivationDue || !got.ReadyForVoteAuthority { t.Fatal("expected readiness at effective height when no other blockers remain") }
	if got.VoteAuthority { t.Fatal("readiness result must never itself grant vote authority") }
}
...
func TestActivationReadinessRejectsMismatchedLocalConsensusKey(t *testing.T) {
	...
	if _, err := evaluateActivationReadiness(cfg, proof, verified, 10); err == nil { t.Fatal("expected governed/local consensus key mismatch to fail") }
}

func TestActivationReadinessRejectsWrongTarget(t *testing.T) {
	cfg, proof, verified := activationReadinessFixture()
	verified.ValidatorID = "validator-e"
	if _, err := evaluateActivationReadiness(cfg, proof, verified, 10); err == nil { t.Fatal("expected wrong activation target to fail") }
}
