...
func verifyProposerCommand(args []string) error {
	...
	if *validatorSetPath==""||*vrfRegistryPath==""||*evidencePath==""||*expectedValidatorRoot==""||*expectedVRFRoot==""{return errors.New("--validator-set, --vrf-registry, --evidence, --validator-set-root and --vrf-registry-root are required")}
	if !isSHA256(*expectedValidatorRoot)||!isSHA256(*expectedVRFRoot){return errors.New("trusted roots must be lowercase SHA-256 digests")}
	var set SnapshotValidatorSet;if err:=readJSON(*validatorSetPath,&set);err!=nil{return err}
	var registry VRFKeyRegistry;if err:=readJSON(*vrfRegistryPath,&registry);err!=nil{return err}
	var evidence ProposerEntropyEvidence;if err:=readJSON(*evidencePath,&evidence);err!=nil{return err}
	result,err:=verifyProposerEntropyPortable(set,registry,evidence,*expectedValidatorRoot,*expectedVRFRoot,*protocolVersion);if err!=nil{return err}
