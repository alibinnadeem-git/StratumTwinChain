...
func consensusSafetyCachePath(dir string) string {
	return filepath.Join(dir, "state", "consensus-safety.json")
}
...
func loadConsensusPrivateKey(dir string) (ed25519.PrivateKey, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("parse CONSENSUS private key: %w", err)
	}
	key, ok := parsed.(ed25519.PrivateKey)
	if !ok {
		return nil, errors.New("CONSENSUS private key is not Ed25519")
	}
	return key, nil
}
...
func consensusPublicKey(cfg BootstrapConfig) (ed25519.PublicKey, error) {
	...
	if !ok || ref.PublicKeyB64 == "" {
		return nil, errors.New("CONSENSUS public key is missing from validator config")
	}
	raw, err := base64.StdEncoding.DecodeString(ref.PublicKeyB64)
	if err != nil {
		return nil, fmt.Errorf("decode CONSENSUS public key: %w", err)
	}
	parsed, err := x509.ParsePKIXPublicKey(raw)
	if err != nil {
		return nil, fmt.Errorf("parse CONSENSUS public key: %w", err)
	}
	key, ok := parsed.(ed25519.PublicKey)
	if !ok {
		return nil, errors.New("CONSENSUS public key is not Ed25519")
	}
	return key, nil
}
...
func verifyConsensusSafetyRecord(cfg BootstrapConfig, previous *ConsensusSafetyRecord, record ConsensusSafetyRecord) error {
	if record.Version != consensusSafetyVersion || record.Domain != consensusSafetyDomain {
		return errors.New("unsupported consensus safety record version/domain")
	}
	if record.ChainID != cfg.ChainID || record.ValidatorID != cfg.ValidatorID {
		return errors.New("consensus safety record identity does not match validator config")
	}
	ref, ok := cfg.Keys["CONSENSUS"]
	if !ok || !strings.EqualFold(record.ConsensusKeyHash, ref.PublicKeyHash) {
		return errors.New("consensus safety record key hash does not match configured CONSENSUS key")
	}
	if previous == nil {
		if record.Sequence != 0 || record.Step != "BOOTSTRAP" || record.Height != 0 || record.Round != 0 || record.NewRound != nil {
			return errors.New("consensus safety journal must begin with BOOTSTRAP sequence 0")
		}
		if !strings.EqualFold(record.PreviousRecordHash, cfg.GenesisDIRHash) {
			return errors.New("BOOTSTRAP safety record must bind the pinned Genesis DIR hash")
		}
		if record.LockedDIR != "" || record.LockedRound != nil || record.ValidDIR != "" || record.ValidRound != nil || record.FinalizedDIRHash != "" {
			return errors.New("BOOTSTRAP safety record must not contain consensus decisions")
		}
	} else {
		if record.Sequence != previous.Sequence+1 {
			return errors.New("consensus safety journal sequence is not contiguous")
		}
		if record.PreviousRecordHash != previous.RecordHash {
			return errors.New("consensus safety journal hash chain is broken")
		}
		if err := validateSafetyTransition(*previous, record); err != nil {
			return err
		}
	}
	...
	if record.RecordHash != expectedHash {
		return errors.New("consensus safety record hash mismatch")
	}
	sig, err := base64.StdEncoding.DecodeString(record.Signature)
	if err != nil {
		return fmt.Errorf("decode consensus safety signature: %w", err)
	}
	...
	if !ed25519.Verify(pub, digest, sig) {
		return errors.New("consensus safety record signature verification failed")
	}
	return nil
}
...
func validateDecisionShape(decision ConsensusSafetyDecision) error {
	...
	if safetyStepRank(decision.Step) < 0 || decision.Step == "BOOTSTRAP" {
		return fmt.Errorf("unsupported consensus safety step %q", decision.Step)
	}
	if decision.Height == 0 {
		return errors.New("consensus safety decision height must be positive")
	}
	if decision.ProposalHash != "" && decision.ProposalHash != "NIL" && !isSHA256(decision.ProposalHash) {
		return errors.New("proposalHash must be NIL or a SHA-256 digest")
	}
	if decision.StateRoot != "" && !isSHA256(decision.StateRoot) {
		return errors.New("stateRoot must be a SHA-256 digest")
	}
	if decision.MessageHash != "" && !isSHA256(decision.MessageHash) {
		return errors.New("messageHash must be a SHA-256 digest")
	}
	if decision.UnlockProofHash != "" && !isSHA256(decision.UnlockProofHash) {
		return errors.New("unlockProofHash must be a SHA-256 digest")
	}
	if decision.FinalizedDIRHash != "" && !isSHA256(decision.FinalizedDIRHash) {
		return errors.New("finalizedDIRHash must be a SHA-256 digest")
	}
	if decision.Step == "ROUND_CHANGE" {
		if decision.NewRound == nil || *decision.NewRound <= decision.Round {
			return errors.New("ROUND_CHANGE requires newRound strictly greater than trigger round")
		}
	...
	case "VERIFY":
		if decision.ProposalHash == "" {
			return errors.New("VERIFY requires proposalHash or NIL")
		}
		if decision.MessageHash == "" {
			return errors.New("VERIFY requires canonical messageHash")
		}
	case "LOCK":
		if !isSHA256(decision.ProposalHash) {
			return errors.New("LOCK requires a non-NIL proposalHash")
		}
	case "COMMIT":
		if !isSHA256(decision.ProposalHash) || decision.MessageHash == "" {
			return errors.New("COMMIT requires proposalHash and canonical messageHash")
		}
	case "ROUND_CHANGE":
		if decision.MessageHash == "" {
			return errors.New("ROUND_CHANGE requires canonical messageHash")
		}
	case "FINALIZE":
		if !isSHA256(decision.ProposalHash) || !isSHA256(decision.FinalizedDIRHash) {
			return errors.New("FINALIZE requires proposalHash and finalizedDIRHash")
		}
	}
	if (decision.UnlockProofHash == "") != (decision.UnlockProofRound == nil) {
		return errors.New("unlock proof hash and round must be supplied together")
	}
	return nil
}

func minimumRoundAfter(record ConsensusSafetyRecord) uint64 {
	if record.Step == "ROUND_CHANGE" && record.NewRound != nil {
		return *record.NewRound
	}
	return record.Round
}
...
func validateSafetyTransition(previous, record ConsensusSafetyRecord) error {
	if record.Height < previous.Height {
		return errors.New("consensus safety height rollback is forbidden")
	}
	heightAdvanced := record.Height > previous.Height
	if !heightAdvanced && record.Round < minimumRoundAfter(previous) {
		return errors.New("consensus safety round rollback is forbidden")
	}
	if !heightAdvanced && record.Round == previous.Round && safetyStepRank(record.Step) <= safetyStepRank(previous.Step) {
		return errors.New("consensus safety step rollback or duplicate journal entry is forbidden")
	}
	if heightAdvanced {
		if previous.Height != 0 && previous.FinalizedDIRHash == "" {
			return errors.New("cannot advance consensus safety height before prior height FINALIZE is durably recorded")
		}
		if record.Step != "VERIFY" && record.Step != "ROUND_CHANGE" {
			return errors.New("new consensus height must begin with a durable VERIFY or ROUND_CHANGE decision")
		}
		if record.LockedDIR != "" || record.LockedRound != nil || record.ValidDIR != "" || record.ValidRound != nil || record.FinalizedDIRHash != "" {
			return errors.New("new consensus height must start with clean lock, valid-value, and finality state")
		}
	}
	if !heightAdvanced && record.Step == "VERIFY" && record.ProposalHash != "NIL" && previous.LockedDIR != "" && record.ProposalHash != previous.LockedDIR {
		if record.UnlockProofRound == nil || record.UnlockProofHash == "" || *record.UnlockProofRound <= valueOrZero(previous.LockedRound) || *record.UnlockProofRound >= record.Round {
			return errors.New("conflicting VERIFY requires higher-round PLC proof between lockedRound and current round")
		}
	}
	if record.Step == "LOCK" {
		if heightAdvanced {
			return errors.New("LOCK cannot be the first durable decision of a new height")
		}
		if record.LockedDIR != record.ProposalHash || record.LockedRound == nil || *record.LockedRound != record.Round {
			return errors.New("LOCK safety record must persist the current proposal and round")
		}
	...
	if record.Step == "COMMIT" {
		if heightAdvanced || previous.LockedDIR == "" || record.ProposalHash != previous.LockedDIR || previous.LockedRound == nil || *previous.LockedRound != record.Round {
			return errors.New("COMMIT safety record must match the current-round durable lock")
		}
	}
	if record.Step == "FINALIZE" {
		if heightAdvanced || previous.LockedDIR == "" || record.ProposalHash != previous.LockedDIR {
			return errors.New("FINALIZE safety record must match the durable lock")
		}
		if safetyStepRank(previous.Step) < safetyStepRank("COMMIT") {
			return errors.New("FINALIZE cannot be recorded before COMMIT")
		}
	}
	return nil
}
...
func readConsensusSafetyHistory(dir string, cfg BootstrapConfig) ([]ConsensusSafetyRecord, error) {
	...
	sort.Strings(names)
	if len(names) == 0 {
		return nil, errors.New("consensus safety journal is empty; run init-consensus-safety")
	}
	...
	for index, name := range names {
		...
		if name != want {
			return nil, fmt.Errorf("consensus safety journal sequence file mismatch: expected %s got %s", want, name)
		}
		var record ConsensusSafetyRecord
		if err := readJSON(filepath.Join(consensusSafetyDir(dir), name), &record); err != nil {
			return nil, err
		}
		if err := verifyConsensusSafetyRecord(cfg, previous, record); err != nil {
			return nil, fmt.Errorf("verify consensus safety sequence %d: %w", index, err)
		}
...
func latestConsensusSafetyRecord(dir string) (BootstrapConfig, ConsensusSafetyRecord, []ConsensusSafetyRecord, error) {
	...
	if err := readJSON(filepath.Join(dir, "config.json"), &cfg); err != nil {
		return cfg, ConsensusSafetyRecord{}, nil, err
	}
...
func initializeConsensusSafety(dir string) error {
	...
	if err := os.MkdirAll(consensusSafetyDir(dir), 0o700); err != nil {
		return err
	}
	entries, err := os.ReadDir(consensusSafetyDir(dir))
	if err != nil {
	...
	ref, ok := cfg.Keys["CONSENSUS"]
	if !ok || ref.PublicKeyHash == "" {
		return errors.New("CONSENSUS key reference missing from validator config")
	}
	...
	if err := writeImmutableSafetyRecord(consensusSafetyRecordPath(dir, 0), record); err != nil {
		return err
	}
...
func recordConsensusSafetyDecision(dir string, decision ConsensusSafetyDecision) (ConsensusSafetyRecord, error) {
	...
	if strings.EqualFold(decision.ProposalHash, "nil") {
		decision.ProposalHash = "NIL"
	}
	...
	if existing, err := findSafetyDecision(records, decision); err != nil {
		return ConsensusSafetyRecord{}, err
	...
	if record.Step == "LOCK" {
		r := record.Round
		record.LockedDIR = record.ProposalHash
		record.LockedRound = &r
		record.ValidDIR = record.ProposalHash
		record.ValidRound = &r
	}
	if record.Step == "VERIFY" && !heightAdvanced && record.ProposalHash != "NIL" && previous.LockedDIR != "" && record.ProposalHash != previous.LockedDIR {
		record.ValidDIR = record.ProposalHash
		record.ValidRound = cloneUint64(record.UnlockProofRound)
	}
	if record.Step == "FINALIZE" {
		record.FinalizedDIRHash = decision.FinalizedDIRHash
	}
	if err := validateSafetyTransition(previous, record); err != nil {
		return ConsensusSafetyRecord{}, err
	}
	...
	if err := writeImmutableSafetyRecord(consensusSafetyRecordPath(dir, record.Sequence), record); err != nil {
		return ConsensusSafetyRecord{}, err
	}
	if err := writeSafetyCache(dir, record); err != nil {
		return ConsensusSafetyRecord{}, fmt.Errorf("safety journal committed but latest cache update failed: %w", err)
	}
	return record, nil
}
...
func prepareConsensusSignature(dir string, decision ConsensusSafetyDecision) (ConsensusSafetyRecord, string, error) {
	...
	if !cfg.VoteAuthority {
		return ConsensusSafetyRecord{}, "", errors.New("vote authority is false; governed activation is required before consensus signing")
	}
	if decision.Step != "VERIFY" && decision.Step != "COMMIT" && decision.Step != "ROUND_CHANGE" {
		return ConsensusSafetyRecord{}, "", errors.New("outbound consensus signing is limited to VERIFY, COMMIT, and ROUND_CHANGE")
	}
	if !isSHA256(decision.MessageHash) {
		return ConsensusSafetyRecord{}, "", errors.New("canonical messageHash is required before consensus signing")
	}
...
func writeImmutableSafetyRecord(path string, record ConsensusSafetyRecord) error {
	if _, err := os.Stat(path); err == nil {
		return errors.New("refusing to overwrite immutable consensus safety record")
...
func recoverConsensusSafetyCache(dir string) (ConsensusSafetyRecord, error) {
	...
	if err := writeSafetyCache(dir, latest); err != nil {
		return ConsensusSafetyRecord{}, err
	}
	return latest, nil
}
...
func initConsensusSafetyCommand(args []string) error {
	...
	if err := initializeConsensusSafety(*dir); err != nil {
		return err
	}
...
func verifyConsensusSafetyCommand(args []string) error {
	...
	if *repairCache {
		if _, err := recoverConsensusSafetyCache(*dir); err != nil {
			return err
		}
	}
