...
func initCommand(args []string) error {
	...
	if *chainID == "" || *networkName == "" || *friendlyLabel == "" {
		return errors.New("--chain-id, --network-name and --friendly-label are required")
	}
	...
	if err := initializeConsensusSafety(*dir); err != nil {
		return fmt.Errorf("initialize signed consensus safety journal: %w", err)
	}
...
func loadEnrollmentCode(path string) (string, error) {
	if env := strings.TrimSpace(os.Getenv("STRATUM_ENROLLMENT_CODE")); env != "" {
		return env, nil
	}
	if path == "" {
		return "", errors.New("supply enrollment secret via STRATUM_ENROLLMENT_CODE or --enrollment-code-file; command-line secret values are intentionally unsupported")
	}
	...
	if runtime.GOOS != "windows" && info.Mode().Perm()&0o077 != 0 {
		return "", errors.New("enrollment-code file permissions must not grant group/other access")
	}
...
func generateLocalKey(dir, purpose string) (KeyRef, error) {
	...
	algorithm := "ED25519"
	if purpose == "VRF" {
		algorithm = "ED25519_CANDIDATE_KEY_MATERIAL"
	}
...
func doctorCommand(args []string) error {
	...
	if !contains(cfg.ActivationBlockedReasons, "GOVERNANCE_ACTIVATION_REQUIRED") {
		problems = append(problems, "missing governance activation block")
	}
	for _, purpose := range []string{"CONSENSUS", "VRF", "TRANSPORT"} {
	...
	if safetyErr != nil {
		problems = append(problems, "signed consensus safety journal verification failed: "+safetyErr.Error())
	}
	if len(problems) > 0 {
...
func verifyPackageCommand(args []string) error {
	...
	binaryPath := fs.String("binary", "", "optional validator binary")
	sbomPath := fs.String("sbom", "", "optional SBOM")
	if err := fs.Parse(args); err != nil {
		return err
	}
	...
	sig, err := base64.StdEncoding.DecodeString(m.PublisherSignature)
	if err != nil {
		return fmt.Errorf("decode publisher signature: %w", err)
	}
	u := unsignedManifest{m.PackageID, m.ValidatorVersion, m.ProtocolVersion, m.Platform, m.Architecture, strings.ToLower(m.BinaryHash), strings.ToLower(m.SBOMHash), m.PublisherKeyID, m.MinimumResources, m.SupportedFeatures, m.ReleaseChannel}
	...
	for _, item := range []struct{ path, expected, label string }{{*binaryPath, m.BinaryHash, "binary"}, {*sbomPath, m.SBOMHash, "SBOM"}} {
		if item.path != "" {
			got, err := hashFile(item.path)
			if err != nil {
				return err
			}
			if got != strings.ToLower(item.expected) {
				return fmt.Errorf("%s hash does not match package manifest", item.label)
			}
		}
	}
