...
func verifyGenesisTrustCommand(args []string) error {
	...
	if *genesisPath == "" || *bundlePath == "" || *bundleHash == "" || *certificatePath == "" {
		return errors.New("--genesis, --trust-bundle, --trust-bundle-hash and --certificate are required")
	}
	if !isSHA256(*bundleHash) {
		return errors.New("--trust-bundle-hash must be a 64-character SHA-256 hex digest")
	}
	var cfg BootstrapConfig
	if err := readJSON(filepath.Join(*dir, "config.json"), &cfg); err != nil {
		return err
	}
	genesis, err := verifyCanonicalGenesis(*genesisPath, cfg.ChainID, cfg.GenesisDIRHash)
	if err != nil {
		return err
	}
	cert, err := verifyGenesisTrust(*bundlePath, *certificatePath, *bundleHash, cfg.ChainID, genesis.ComputedHash, cfg.ProtocolVersion)
	if err != nil {
		return err
	}
