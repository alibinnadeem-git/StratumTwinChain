...
func activeSnapshotConsensusKey(member SnapshotValidator, height int64) (SnapshotConsensusKey, error) {
	...
	for _, key := range member.Keys {
		if key.Purpose != "CONSENSUS" || key.Algorithm != "Ed25519" {
			continue
		}
	...
	if len(active) != 1 {
		return SnapshotConsensusKey{}, fmt.Errorf("validator %s must have exactly one active CONSENSUS key at height %d; found %d", member.ValidatorID, height, len(active))
	}
	return active[0], nil
}
...
func snapshotValidatorSetRoot(set SnapshotValidatorSet, height int64) (string, error) {
	if set.SetVersion != validatorSetProfile {
		return "", fmt.Errorf("unsupported validator set profile %q", set.SetVersion)
	}
	if set.ChainID == "" {
		return "", errors.New("validator set chainId is required")
	}
	validators := make([]map[string]any, 0, len(set.Members))
	seen := map[string]bool{}
	for _, member := range set.Members {
		if seen[member.ValidatorID] {
			return "", fmt.Errorf("duplicate validatorId %s", member.ValidatorID)
		}
	...
	if len(validators) == 0 {
		return "", fmt.Errorf("no ACTIVE validators at height %d", height)
	}
	...
	normalized, err := canonicalValue(wrapped)
	if err != nil {
		return "", err
	}
	preimage, err := canonicalJSON(normalized)
	if err != nil {
		return "", err
	}
...
func canonicalSnapshotPayload(cert SnapshotTrustCertificate) ([]byte, error) {
	normalized, err := canonicalValue(snapshotPayload(cert))
	if err != nil {
		return nil, err
	}
...
func requiredSnapshotQuorum(n int) (int, error) {
	if n < 1 {
		return 0, errors.New("snapshot requires at least one ACTIVE validator")
	}
	return (2*n)/3 + 1, nil
}
...
func verifySnapshotCertificate(set SnapshotValidatorSet, cert SnapshotTrustCertificate, expectedChainID, expectedValidatorSetRoot, expectedProtocolVersion string) (SnapshotVerification, error) {
	if cert.Domain != snapshotCertificateDomain || cert.CertificateVersion != snapshotCertificateVersion {
		return SnapshotVerification{}, errors.New("unsupported snapshot certificate domain/version")
	}
	if set.ChainID != expectedChainID || cert.ChainID != expectedChainID {
		return SnapshotVerification{}, errors.New("snapshot chainId mismatch")
	}
	if expectedProtocolVersion != "" && cert.ProtocolVersion != expectedProtocolVersion {
		return SnapshotVerification{}, errors.New("snapshot protocolVersion mismatch")
	}
	for label, value := range map[string]string{"DIRHash": cert.DIRHash, "stateRoot": cert.StateRoot, "validatorSetRoot": cert.ValidatorSetRoot, "stateManifestRoot": cert.StateManifestRoot, "SpatialManifestRoot": cert.SpatialManifestRoot, "evidenceManifestRoot": cert.EvidenceManifestRoot} {
		if !isSHA256(value) {
			return SnapshotVerification{}, fmt.Errorf("%s must be a lowercase SHA-256 digest", label)
		}
	}
	computedRoot, err := snapshotValidatorSetRoot(set, cert.SnapshotHeight)
	if err != nil {
		return SnapshotVerification{}, err
	}
	if computedRoot != expectedValidatorSetRoot {
		return SnapshotVerification{}, fmt.Errorf("trusted validator-set root mismatch: computed %s", computedRoot)
	}
	if cert.ValidatorSetRoot != computedRoot {
		return SnapshotVerification{}, errors.New("snapshot certificate does not bind the trusted validator-set root")
	}

	active := map[string]SnapshotValidator{}
	for _, member := range set.Members {
		if snapshotValidatorActive(member, cert.SnapshotHeight) {
	...
	payload, err := canonicalSnapshotPayload(cert)
	if err != nil {
		return SnapshotVerification{}, err
	}
	valid := map[string]bool{}
	for _, sig := range cert.Signatures {
		if valid[sig.SignerID] {
			continue
		}
		member, ok := active[sig.SignerID]
		if !ok {
			continue
		}
		key, err := activeSnapshotConsensusKey(member, cert.SnapshotHeight)
		if err != nil {
			return SnapshotVerification{}, err
		}
		if sig.KeyID != key.KeyID || sig.Algorithm != "Ed25519" || sig.Domain != snapshotCertificateDomain {
			continue
		}
	...
	if len(validSigners) < required {
		return SnapshotVerification{}, fmt.Errorf("snapshot PoVI quorum not met: %d/%d; %d required", len(validSigners), len(active), required)
	}
