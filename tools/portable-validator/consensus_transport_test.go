...
func makeConsensusTransportFixture(t *testing.T, active bool) consensusTransportFixture {
	...
	if err := os.WriteFile(filepath.Join(dir, "keys", "private", "consensus.pk8"), privDER, 0o600); err != nil {
		t.Fatal(err)
	}
	state := candidateState
	voteAuthority := false
	blockers := []string{"GOVERNANCE_ACTIVATION_REQUIRED", "LIVE_POVI_NETWORK_EXECUTION_NOT_YET_IMPLEMENTED"}
	if active {
		state = "ACTIVE"
		voteAuthority = true
		blockers = []string{}
	...
	root, err := snapshotValidatorSetRoot(set, 1)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCandidateCannotSignConsensusWire(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "ACTIVE state") {
		t.Fatalf("expected governed ACTIVE rejection, got %v", err)
	}
	_, _, records, err := latestConsensusSafetyRecord(f.dir)
	if err != nil {
		t.Fatal(err)
	}
	if len(records) != 1 || records[0].Step != "BOOTSTRAP" {
		t.Fatalf("candidate signing attempt mutated safety journal: %+v", records)
	}
}
...
func TestVerifyWirePersistsIntentAndBlocksEquivocation(t *testing.T) {
	...
	wire, err := signVerifyWireMessage(f.dir, f.set, 1, 0, proposal, f.root, "POVI/1", "", nil)
	if err != nil {
		t.Fatal(err)
	}
	if err := verifyConsensusWireMessage(f.set, wire, f.cfg.ChainID, f.root, "POVI/1"); err != nil {
		t.Fatal(err)
	}
	...
	if len(records) != 2 || latest.Step != "VERIFY" || latest.MessageHash != wire.Verify.MessageHash {
		t.Fatalf("VERIFY was not durably recorded before return: %+v", latest)
	}
	...
	if err == nil || !strings.Contains(err.Error(), "EQUIVOCATION_BLOCKED") {
		t.Fatalf("expected same-round equivocation block, got %v", err)
	}
	_, _, records, err = latestConsensusSafetyRecord(f.dir)
	if err != nil {
		t.Fatal(err)
	...
	if len(records) != 2 {
		t.Fatalf("equivocation attempt appended journal record: %d", len(records))
	}
}
...
func TestCommitWireRequiresDurableLockAndVerifies(t *testing.T) {
	...
	if _, err := recordConsensusSafetyDecision(f.dir, ConsensusSafetyDecision{Height: 1, Round: 0, Step: "LOCK", ProposalHash: proposal}); err != nil {
		t.Fatal(err)
	}
	wire, err := signCommitWireMessage(f.dir, f.set, 1, 0, proposal, stateRoot, f.root, "POVI/1")
	if err != nil {
		t.Fatal(err)
	}
	if err := verifyConsensusWireMessage(f.set, wire, f.cfg.ChainID, f.root, "POVI/1"); err != nil {
	...
	_, latest, _, err := latestConsensusSafetyRecord(f.dir)
	if err != nil {
		t.Fatal(err)
	}
	if latest.Step != "COMMIT" || latest.ProposalHash != proposal || latest.MessageHash != wire.Commit.MessageHash {
		t.Fatalf("COMMIT not journal-bound: %+v", latest)
	}
}
...
func TestRoundChangeWireUsesRecoveredSafetyState(t *testing.T) {
	...
	wire, err := signRoundChangeWireMessage(f.dir, f.set, 1, 0, 1, f.root, "POVI/1", nil)
	if err != nil {
		t.Fatal(err)
	}
	if wire.TriggerRound == nil || *wire.TriggerRound != 0 {
		t.Fatalf("missing trigger round")
	}
	if err := verifyConsensusWireMessage(f.set, wire, f.cfg.ChainID, f.root, "POVI/1"); err != nil {
		t.Fatal(err)
	}
	...
	if latest.Step != "ROUND_CHANGE" || latest.NewRound == nil || *latest.NewRound != 1 {
		t.Fatalf("ROUND_CHANGE not durably journaled: %+v", latest)
	}
}

func TestConsensusWireRejectsTamper(t *testing.T) {
	f := makeConsensusTransportFixture(t, true)
	wire, err := signVerifyWireMessage(f.dir, f.set, 1, 0, strings.Repeat("b", 64), f.root, "POVI/1", "", nil)
	if err != nil {
		t.Fatal(err)
	}
	wire.Verify.ProposalHash = strings.Repeat("c", 64)
	if err := verifyConsensusWireMessage(f.set, wire, f.cfg.ChainID, f.root, "POVI/1"); err == nil {
		t.Fatal("tampered VERIFY unexpectedly verified")
	}
}
