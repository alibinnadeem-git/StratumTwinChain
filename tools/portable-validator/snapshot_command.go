...
func verifySnapshotCommand(args []string) error {
	...
	if *certificatePath == "" || *validatorSetPath == "" {
		return errors.New("--certificate and --validator-set are required")
	}
	if !isSHA256(strings.ToLower(*validatorSetRoot)) {
		return errors.New("--validator-set-root must be a 64-character SHA-256 hex digest from an independently trusted history")
	}
