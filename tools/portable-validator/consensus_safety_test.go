...
func TestConsensusSafetyJournalInitializesSignedBootstrapAndCandidateCannotSign(t *testing.T) {
	...
	if len(records) != 1 || latest.Sequence != 0 || latest.Step != "BOOTSTRAP" {
		t.Fatalf("unexpected bootstrap journal: %+v", records)
	}
	if latest.PreviousRecordHash != cfg.GenesisDIRHash {
		t.Fatal("bootstrap record does not bind pinned Genesis DIR hash")
	}
	if latest.RecordHash == "" || latest.Signature == "" {
		t.Fatal("bootstrap safety record is not signed")
	}
	if latest.VoteAuthority {
		t.Fatal("candidate bootstrap must not gain vote authority")
	}
	...
	if err == nil || !strings.Contains(err.Error(), "vote authority is false") {
		t.Fatalf("candidate consensus signing should be denied, got %v", err)
	}
}
...
func TestConsensusSafetyBlocksSameRoundEquivocationAfterRestart(t *testing.T) {
	...
	if record1.Sequence != 1 || sig1 == "" {
		t.Fatal("VERIFY was not durably persisted before signing")
	}
	_, recovered, records, err := latestConsensusSafetyRecord(dir) // restart/reload simulation
	if err != nil {
		t.Fatal(err)
	}
	if len(records) != 2 || recovered.RecordHash != record1.RecordHash {
		t.Fatal("restart did not recover durable VERIFY history")
	}
	record2, sig2, err := prepareConsensusSignature(dir, first)
	if err != nil {
		t.Fatal(err)
	}
	if record2.RecordHash != record1.RecordHash || sig2 != sig1 {
		t.Fatal("idempotent replay changed the persisted decision or Ed25519 signature")
	}
	conflict := first
...
func TestConsensusSafetyPreservesLockAcrossRestartAndRequiresHigherRoundPLC(t *testing.T) {
	...
	if _, err := recordConsensusSafetyDecision(dir, ConsensusSafetyDecision{Height: 1, Round: 0, Step: "VERIFY", ProposalHash: proposalA, MessageHash: safetyHex("d")}); err != nil {
		t.Fatal(err)
	}
	...
	if locked.LockedDIR != proposalA || locked.LockedRound == nil || *locked.LockedRound != 0 {
		t.Fatal("round-0 lock was not durably recorded")
	}
	if _, _, _, err := latestConsensusSafetyRecord(dir); err != nil {
		t.Fatal(err)
	} // restart/reload
	if _, err := recordConsensusSafetyDecision(dir, ConsensusSafetyDecision{Height: 1, Round: 0, NewRound: safetyRound(1), Step: "ROUND_CHANGE", MessageHash: safetyHex("e")}); err != nil {
		t.Fatal(err)
	}
	if _, err := recordConsensusSafetyDecision(dir, ConsensusSafetyDecision{Height: 1, Round: 1, NewRound: safetyRound(2), Step: "ROUND_CHANGE", MessageHash: safetyHex("f")}); err != nil {
		t.Fatal(err)
	}
	...
	if verifiedB.LockedDIR != proposalA || verifiedB.LockedRound == nil || *verifiedB.LockedRound != 0 {
		t.Fatal("higher-round PLC must not erase the old lock before fresh VERIFY quorum")
	}
	if verifiedB.ValidDIR != proposalB || verifiedB.ValidRound == nil || *verifiedB.ValidRound != 1 {
		t.Fatal("PLC-backed valid value was not recovered")
	}
	lockedB, err := recordConsensusSafetyDecision(dir, ConsensusSafetyDecision{Height: 1, Round: 2, Step: "LOCK", ProposalHash: proposalB})
	if err != nil {
		t.Fatal(err)
	}
	if lockedB.LockedDIR != proposalB || lockedB.LockedRound == nil || *lockedB.LockedRound != 2 {
		t.Fatal("fresh round-2 lock did not replace prior lock")
	}
	if _, err := recordConsensusSafetyDecision(dir, ConsensusSafetyDecision{Height: 1, Round: 2, Step: "COMMIT", ProposalHash: proposalB, MessageHash: safetyHex("3")}); err != nil {
		t.Fatal(err)
	}
...
func TestConsensusSafetyRecoversFromCacheLossUsingImmutableJournal(t *testing.T) {
	...
	if err := os.Remove(consensusSafetyCachePath(dir)); err != nil {
		t.Fatal(err)
	}
	...
	if latest.RecordHash != record.RecordHash || len(records) != 2 {
		t.Fatal("immutable journal did not recover after cache loss")
	}
	recovered, err := recoverConsensusSafetyCache(dir)
	if err != nil {
		t.Fatal(err)
	}
	if recovered.RecordHash != record.RecordHash {
		t.Fatal("cache repair recovered the wrong record")
	}
	var cached ConsensusSafetyRecord
	if err := readJSON(consensusSafetyCachePath(dir), &cached); err != nil {
		t.Fatal(err)
	}
	if cached.RecordHash != record.RecordHash {
		t.Fatal("repaired cache does not match immutable journal")
	}
}

func TestConsensusSafetyRejectsTamperedImmutableHistory(t *testing.T) {
	dir := initSafetyTestValidator(t)
	if _, err := recordConsensusSafetyDecision(dir, ConsensusSafetyDecision{Height: 1, Round: 0, Step: "VERIFY", ProposalHash: safetyHex("b"), MessageHash: safetyHex("c")}); err != nil {
		t.Fatal(err)
	}
	path := consensusSafetyRecordPath(dir, 1)
	var tampered ConsensusSafetyRecord
	if err := readJSON(path, &tampered); err != nil {
		t.Fatal(err)
	}
	tampered.ProposalHash = safetyHex("d")
	if err := writeJSON(path, tampered, 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestConsensusSafetyResetsLocksOnlyAfterDurablePriorHeightFinality(t *testing.T) {
	...
	if _, err := recordConsensusSafetyDecision(dir, ConsensusSafetyDecision{Height: 1, Round: 0, Step: "LOCK", ProposalHash: proposalA}); err != nil {
		t.Fatal(err)
	}
	if _, err := recordConsensusSafetyDecision(dir, ConsensusSafetyDecision{Height: 1, Round: 0, Step: "COMMIT", ProposalHash: proposalA, MessageHash: safetyHex("e")}); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := recordConsensusSafetyDecision(dir, ConsensusSafetyDecision{Height: 1, Round: 0, Step: "FINALIZE", ProposalHash: proposalA, FinalizedDIRHash: finalHash}); err != nil {
		t.Fatal(err)
	}
