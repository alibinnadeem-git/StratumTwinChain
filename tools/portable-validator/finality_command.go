...
func verifyFinalityCommand(args []string) error {
	...
	if *proofPath==""||*validatorSetPath==""||*previousHeight==""{return errors.New("--proof, --validator-set, and --previous-height are required")}
	if !isSHA256(strings.ToLower(*validatorSetRoot)){return errors.New("--validator-set-root must be an independently trusted SHA-256 digest")}
	if !isSHA256(strings.ToLower(*previousDIRHash)){return errors.New("--previous-dir-hash must be a trusted SHA-256 digest")}
	height,err:=strconv.ParseInt(*previousHeight,10,64);if err!=nil||height<0{return errors.New("--previous-height must be a non-negative integer")}
	...
	proof,err:=readFinalityProof(*proofPath);if err!=nil{return err}
	verified,err:=verifyDIRFinalityProof(set,proof,cfg.ChainID,height,strings.ToLower(*previousDIRHash),strings.ToLower(*validatorSetRoot),*protocolVersion);if err!=nil{return err}
