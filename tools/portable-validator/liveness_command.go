...
func verifyRoundChangeCommand(args []string) error {
	...
	if *evidencePath == "" || *validatorSetPath == "" {
		return errors.New("--evidence and --validator-set are required")
	}
	if *currentRound < 0 {
		return errors.New("--current-round must be a non-negative integer")
	}
	if !isSHA256(strings.ToLower(*validatorSetRoot)) {
		return errors.New("--validator-set-root must be a 64-character SHA-256 digest from independently trusted validator history")
	}
	...
	evidence, err := readRoundChangeEvidence(*evidencePath)
	if err != nil {
		return err
	}
	...
	if verification.NILQuorumVerified {
		fmt.Printf("OK: prior-round NIL VERIFY quorum independently verified for round %d\n", verification.TriggerRound)
	}
