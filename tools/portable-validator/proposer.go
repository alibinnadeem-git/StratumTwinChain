...
func selectPortableProposer(set SnapshotValidatorSet, selection ProposerSelectionInput, expectedRoot string) (ProposerSelectionResult, error) {
	...
	activeIDs := []string{}
	for _, member := range set.Members { if snapshotValidatorActive(member, selection.Height) { activeIDs = append(activeIDs, member.ValidatorID) } }
	sort.Strings(activeIDs)
	seen := map[string]bool{}
	candidateIDs := make([]string, 0, len(selection.Candidates))
	scores := make([]ProposerScore, 0, len(selection.Candidates))
	for _, candidate := range selection.Candidates {
		...
		if seen[candidate.ValidatorID] { return ProposerSelectionResult{}, fmt.Errorf("duplicate proposer candidate %s", candidate.ValidatorID) }
		seen[candidate.ValidatorID] = true; candidateIDs = append(candidateIDs, candidate.ValidatorID)
		score, err := proposerScore(selection, candidate); if err != nil { return ProposerSelectionResult{}, err }
		scores = append(scores, ProposerScore{candidate.ValidatorID, score})
	}
	sort.Strings(candidateIDs)
	if len(activeIDs) != len(candidateIDs) { return ProposerSelectionResult{}, errors.New("equal eligible opportunity requires exactly one candidate for every ACTIVE validator") }
