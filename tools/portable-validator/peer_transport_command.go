...
func verifyPeerEnvelopeCommand(args []string) error {
	...
	if *registryPath == "" || *envelopePath == "" || *expectedRoot == "" {
		return errors.New("--peer-registry, --envelope and --peer-registry-root are required")
	}
	if *height < 0 {
		return errors.New("--height must be non-negative")
	}
	if !isSHA256(*expectedRoot) {
		return errors.New("--peer-registry-root must be a SHA-256 digest")
	}
	...
	if err := readJSON(*envelopePath, &envelope); err != nil {
		return err
	}
	result, err := verifyPeerEnvelope(registry, envelope, *height, *expectedRoot, time.Now().UTC(), *maxSkew)
	if err != nil {
		return err
	}
	if *replayStatePath != "" {
		state, err := loadPeerReplayState(*replayStatePath, registry.ChainID)
		if err != nil {
			return err
		}
		prunePeerReplayState(&state, time.Now().UTC())
		if err := applyPeerReplayProtection(&state, envelope); err != nil {
			return err
		}
		if err := os.MkdirAll(filepath.Dir(*replayStatePath), 0o700); err != nil {
			return err
		}
		if err := writeJSON(*replayStatePath, state, 0o600); err != nil {
			return err
		}
	}
