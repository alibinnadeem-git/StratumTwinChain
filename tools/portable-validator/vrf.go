...
func activeVRFKeyPortable(registry VRFKeyRegistry, validatorID string, height int64) (VRFKey, error) {
	for _, validator := range registry.Validators {
		...
		for _, key := range validator.Keys {
			if vrfKeyActiveAtHeight(key, height) {
				active = append(active, key)
			}
		}
		if len(active) != 1 {
			return VRFKey{}, fmt.Errorf("validator %s must have exactly one active VRF key at height %d; found %d", validatorID, height, len(active))
		}
		key := active[0]
		if key.Purpose != "VRF" || key.Algorithm != "Ed25519-Deterministic-Entropy-v1" {
			return VRFKey{}, fmt.Errorf("validator %s has unsupported VRF key profile", validatorID)
		}
		return key, nil
	}
...
func vrfKeyRegistryRootPortable(registry VRFKeyRegistry, height int64) (string, error) {
	if registry.RegistryVersion != poviVRFRegistryProfile {
		return "", errors.New("unsupported VRF key registry profile")
	}
	if registry.ChainID == "" || height < 0 {
		return "", errors.New("invalid VRF key registry context")
	}
	...
	for _, item := range registry.Validators {
		if item.ValidatorID == "" || seen[item.ValidatorID] {
			return "", errors.New("VRF key registry validator IDs must be unique and non-empty")
		}
		seen[item.ValidatorID] = true
		key, err := activeVRFKeyPortable(registry, item.ValidatorID, height)
		if err != nil {
			return "", err
		}
	...
	sort.Slice(validators, func(i, j int) bool {
		return validators[i]["validatorId"].(string) < validators[j]["validatorId"].(string)
	})
...
func proposerSelectionSeedPortable(context ProposerSelectionContext) (string, error) {
	if context.ProfileVersion != poviProposerProfile {
		return "", errors.New("unsupported proposer profile")
	}
	if context.Height < 1 || context.Round < 0 || !isSHA256(context.PreviousDIRHash) || !isSHA256(context.PreviousEntropy) || !isSHA256(context.ValidatorSetRoot) || !isSHA256(context.VRFKeyRegistryRoot) {
		return "", errors.New("invalid proposer-selection context")
	}
...
func selectProposerPortable(set SnapshotValidatorSet, context ProposerSelectionContext) (ProposerSelection, error) {
	if set.ChainID != context.ChainID {
		return ProposerSelection{}, errors.New("proposer-selection chainId mismatch")
	}
	root, err := snapshotValidatorSetRoot(set, context.Height)
	if err != nil {
		return ProposerSelection{}, err
	}
	if root != context.ValidatorSetRoot {
		return ProposerSelection{}, fmt.Errorf("proposer-selection validator-set root mismatch: computed %s", root)
	}
	ids := make([]string, 0, len(set.Members))
	for _, member := range set.Members {
		if snapshotValidatorActive(member, context.Height) {
			ids = append(ids, member.ValidatorID)
		}
	}
	sort.Strings(ids)
	if len(ids) < 1 {
		return ProposerSelection{}, fmt.Errorf("no ACTIVE validators at height %d", context.Height)
	}
	seed, err := proposerSelectionSeedPortable(context)
	if err != nil {
		return ProposerSelection{}, err
	}
	seedInt := new(big.Int)
	if _, ok := seedInt.SetString(seed, 16); !ok {
		return ProposerSelection{}, errors.New("invalid proposer selection seed")
	}
...
func verifyProposerEntropyPortable(set SnapshotValidatorSet, registry VRFKeyRegistry, e ProposerEntropyEvidence, expectedValidatorRoot, expectedVRFRoot, expectedProtocol string) (ProposerEntropyVerification, error) {
	if e.ProfileVersion != poviProposerProfile || e.Domain != poviVRFEvidenceDomain {
		return ProposerEntropyVerification{}, errors.New("unsupported proposer entropy profile")
	}
	if set.ChainID != e.ChainID || registry.ChainID != e.ChainID {
		return ProposerEntropyVerification{}, errors.New("proposer entropy chainId mismatch")
	}
	if expectedProtocol != "" && e.ProtocolVersion != expectedProtocol {
		return ProposerEntropyVerification{}, errors.New("proposer entropy protocolVersion mismatch")
	}
	root, err := snapshotValidatorSetRoot(set, e.Height)
	if err != nil {
		return ProposerEntropyVerification{}, err
	}
	if root != expectedValidatorRoot || e.ValidatorSetRoot != root {
		return ProposerEntropyVerification{}, errors.New("proposer entropy validator-set root mismatch")
	}
	vrfRoot, err := vrfKeyRegistryRootPortable(registry, e.Height)
	if err != nil {
		return ProposerEntropyVerification{}, err
	}
	if vrfRoot != expectedVRFRoot || e.VRFKeyRegistryRoot != vrfRoot {
		return ProposerEntropyVerification{}, errors.New("proposer entropy VRF-key registry root mismatch")
	}
	activeIDs := []string{}
	for _, m := range set.Members {
		if snapshotValidatorActive(m, e.Height) {
			activeIDs = append(activeIDs, m.ValidatorID)
		}
	}
	sort.Strings(activeIDs)
	registryIDs := make([]string, 0, len(registry.Validators))
	for _, v := range registry.Validators {
		registryIDs = append(registryIDs, v.ValidatorID)
	}
	sort.Strings(registryIDs)
	if len(activeIDs) != len(registryIDs) {
		return ProposerEntropyVerification{}, errors.New("VRF-key registry must cover exactly the ACTIVE validator set")
	}
	for i := range activeIDs {
		if activeIDs[i] != registryIDs[i] {
			return ProposerEntropyVerification{}, errors.New("VRF-key registry must cover exactly the ACTIVE validator set")
		}
	}
	...
	selected, err := selectProposerPortable(set, context)
	if err != nil {
		return ProposerEntropyVerification{}, err
	}
	if e.SelectionSeed != selected.SelectionSeed {
		return ProposerEntropyVerification{}, errors.New("proposer entropy selectionSeed mismatch")
	}
	if e.ProposerID != selected.ProposerID {
		return ProposerEntropyVerification{}, fmt.Errorf("wrong proposer: expected %s, got %s", selected.ProposerID, e.ProposerID)
	}
	key, err := activeVRFKeyPortable(registry, e.ProposerID, e.Height)
	if err != nil {
		return ProposerEntropyVerification{}, err
	}
	if e.KeyID != key.KeyID {
		return ProposerEntropyVerification{}, errors.New("proposer entropy keyId mismatch")
	}
	msg, err := proposerEntropyMessageHashPortable(e)
	if err != nil {
		return ProposerEntropyVerification{}, err
	}
	if e.MessageHash != msg {
		return ProposerEntropyVerification{}, errors.New("proposer entropy messageHash mismatch")
	}
	...
	if !ok {
		return ProposerEntropyVerification{}, errors.New("VRF profile requires Ed25519 key material")
	}
	proof, err := base64.StdEncoding.DecodeString(e.ProofB64)
	if err != nil {
		return ProposerEntropyVerification{}, errors.New("invalid proposer entropy proof encoding")
	}
	messageBytes, err := hex.DecodeString(msg)
	if err != nil {
		return ProposerEntropyVerification{}, err
	}
	if !ed25519.Verify(pub, messageBytes, proof) {
		return ProposerEntropyVerification{}, errors.New("proposer entropy proof signature verification failed")
	}
	digest := sha256.Sum256(proof)
	output := hex.EncodeToString(digest[:])
	if e.VRFOutput != output {
		return ProposerEntropyVerification{}, errors.New("proposer entropy output mismatch")
	}
