...
func verifyPLCCommand(args []string) error {
 ...
 if *plcPath==""||*validatorSetPath==""||*chainID==""||*validatorSetRoot==""{return errors.New("--plc, --validator-set, --chain-id and --validator-set-root are required")}
 var plc PoVILockCertificateProof; if err:=readJSON(*plcPath,&plc);err!=nil{return err}
 var set SnapshotValidatorSet; if err:=readJSON(*validatorSetPath,&set);err!=nil{return err}
 result,err:=verifyPLC(set,plc,*chainID,*validatorSetRoot,*protocolVersion); if err!=nil{return err}
 out,err:=json.MarshalIndent(result,"","  ");if err!=nil{return err};fmt.Fprintln(os.Stdout,string(out));return nil
}
