...
const valid=new Set();
for(const vote of PFC.COMMITSignatures){const member=members.get(vote.validatorId);if(!member)continue;const key=keyAt(member,header.height);if(vote.proposalHash!==proposalHash||vote.stateRoot!==header.stateRoot||vote.messageHash!==messageHash)continue;const pub=crypto.createPublicKey({key:Buffer.from(key.publicKeyDerB64,'base64'),format:'der',type:'spki'});if(crypto.verify(null,Buffer.from(messageHash,'hex'),pub,Buffer.from(vote.signature,'base64')))valid.add(vote.validatorId);}
const required=quorum(members.size);if(required!==3||valid.size<required)throw new Error(`PFC quorum failed ${valid.size}/${required}`);
const signerIds=[...valid].sort();if(JSON.stringify(signerIds)!==JSON.stringify(PFC.signerIds))throw new Error('PFC signer set mismatch');
const DIRHash=hash({domain:'STRATUM/DIR/1',header,finality:{proposalHash,stateRoot:header.stateRoot,validatorSetRoot:root,protocolVersion:header.protocolVersion,signerIds}});if(DIRHash!==PFC.DIRHash)throw new Error(`DIR hash mismatch ${DIRHash}`);
if(PFC.COMMITSignatures.slice(0,2).length>=required)throw new Error('2-of-3 must be below quorum');
const mutatedHeader={...header,previousDIRHash:'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'};if(hash({domain:'STRATUM/POVI/PROPOSAL/1',DIRCandidateHeader:mutatedHeader})===proposalHash)throw new Error('header mutation did not change proposal hash');
const implementation=fs.readFileSync('lib/redbook/finality-proof.ts','utf8');for(const token of ['POVI_COMMIT_DOMAIN','proposalHashForDIRHeader','trustedPreviousDIRHash','PFC PoVI quorum not met','PFC signerIds do not exactly match valid COMMIT signatures','DIRHash mismatch'])if(!implementation.includes(token))throw new Error(`Missing finality invariant: ${token}`);
